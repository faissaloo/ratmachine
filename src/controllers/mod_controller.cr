...
class ModController < ApplicationController
  ...
  def render_panel()
    content(element_name: :div, options: { class: "panel" }.to_h) do
      content(element_name: :a, options: { href: "/mod/delete?id=#{ params[:id]? }" }.to_h) do
        "Delete posts"
      end +
      "<br/>" +
      content(element_name: :a, options: { href: "/mod/filter?id=#{ params[:id]? }" }.to_h) do
        "Manage post filters"
      end +
      "<br/>"+
      content(element_name: :a, options: { href: "/mod/bans?id=#{ params[:id]? }" }.to_h) do
        "Manage bans"
      end +
      "<br/>"+
